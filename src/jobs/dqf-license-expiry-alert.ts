...
export interface DqfLicenseExpiryAlertData {
  tpaOrgId?: string;
}

/**
 * Fires daily. Queries driver qualifications where qualificationType = 'cdl',
 * expiresAt is within 30 days, and status = 'active'.
 * Updates status to 'expiring_soon' and sends alerts.
 */
export async function handleDqfLicenseExpiryAlert(job: Job<DqfLicenseExpiryAlertData>) {
  ...
  if (enabledTenants.length === 0) {
    console.log('[dqf-license-expiry-alert] No tenants with DQF enabled — skipping');
    return;
  }
  ...
  for (const tenant of enabledTenants) {
    const expiringLicenses = await db.query.driverQualifications.findMany({
      where: and(
        eq(driverQualifications.tpaOrgId, tenant.tpaOrgId),
        eq(driverQualifications.qualificationType, 'cdl'),
    ...
    if (expiringLicenses.length === 0) continue;

    // Update status to expiring_soon
    for (const qual of expiringLicenses) {
    ...
    for (const qual of expiringLicenses) {
      ...
      for (const user of staffUsers) {
        await createNotification({
          userId: user.id,
          type: 'general',
          title: `CDL License Expiring — ${personName}`,
          message: `CDL license for ${personName} expires on ${expiresAt}`,
      ...
      if (recipientEmails.length > 0) {
        await sendLicenseExpiryAlert({
          tpaOrgId: tenant.tpaOrgId,
          personName,
          qualificationType: 'CDL License',
          expiresAt,
          recipientEmails,
          branding,
        }).catch(err => console.error('[dqf-license-expiry-alert] Email failed:', err));
      }
  ...
  console.log(`[dqf-license-expiry-alert] Processed ${totalProcessed} expiring CDL licenses`);
}
