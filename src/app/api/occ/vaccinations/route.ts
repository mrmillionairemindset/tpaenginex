import { NextResponse } from 'next/server';
import { db } from '@/db';
import { vaccinations, persons } from '@/db/schema';
import { withPermission } from '@/auth/api-middleware';
import { and, desc, eq, count } from 'drizzle-orm';
import { parsePagination } from '@/lib/pagination';
import { z } from 'zod';
import { createAuditLog } from '@/lib/audit';

export const dynamic = 'force-dynamic';

const createSchema = z.object({
  personId: z.string().uuid(),
  vaccineType: z.string().min(1).max(100),
  manufacturer: z.string().max(100).optional(),
  lotNumber: z.string().max(100).optional(),
  administeredAt: z.string().datetime().optional(),
  doseNumber: z.number().int().min(1).max(20).optional(),
  expiresAt: z.string().datetime().optional(),
  notes: z.string().max(5000).optional(),
});

export const GET = withPermission('view_vaccinations', async (req, user) => {
  const { searchParams } = new URL(req.url);
  const tpaOrgId = user.tpaOrgId;
  const { page, limit, offset } = parsePagination(searchParams);
  const personId = searchParams.get('personId');
  const vaccineType = searchParams.get('vaccineType');

  let where: any = tpaOrgId ? eq(vaccinations.tpaOrgId, tpaOrgId) : undefined;
  if (personId)
    where = where ? and(where, eq(vaccinations.personId, personId)) : eq(vaccinations.personId, personId);
  if (vaccineType)
    where = where
      ? and(where, eq(vaccinations.vaccineType, vaccineType))
      : eq(vaccinations.vaccineType, vaccineType);

  const [rows, totalRows] = await Promise.all([
    db.query.vaccinations.findMany({
      where,
      with: {
        person: { columns: { id: true, firstName: true, lastName: true } },
      },
      orderBy: [desc(vaccinations.administeredAt)],
      limit,
      offset,
    }),
    db.select({ count: count() }).from(vaccinations).where(where),
  ]);

  const total = Number(totalRows[0]?.count ?? 0);
  return NextResponse.json({
    vaccinations: rows,
    pagination: { page, limit, total, totalPages: Math.ceil(total / limit), hasMore: offset + rows.length < total },
  });
});

export const POST = withPermission('manage_vaccinations', async (req, user) => {
  const tpaOrgId = user.tpaOrgId;
  if (!tpaOrgId) return NextResponse.json({ error: 'TPA context required' }, { status: 400 });

  const body = await req.json().catch(() => null);
  const parsed = createSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: 'Invalid request', details: parsed.error.errors }, { status: 400 });
  }

  const person = await db.query.persons.findFirst({
    where: and(eq(persons.id, parsed.data.personId), eq(persons.tpaOrgId, tpaOrgId)),
  });
  if (!person) return NextResponse.json({ error: 'Person not found' }, { status: 404 });

  const [row] = await db
    .insert(vaccinations)
    .values({
      tpaOrgId,
      personId: parsed.data.personId,
      vaccineType: parsed.data.vaccineType,
      manufacturer: parsed.data.manufacturer || null,
      lotNumber: parsed.data.lotNumber || null,
      administeredAt: parsed.data.administeredAt ? new Date(parsed.data.administeredAt) : new Date(),
      administeredBy: user.id,
      doseNumber: parsed.data.doseNumber || null,
      expiresAt: parsed.data.expiresAt ? new Date(parsed.data.expiresAt) : null,
      notes: parsed.data.notes || null,
    })
    .returning();

  await createAuditLog({
    tpaOrgId,
    actorUserId: user.id,
    actorEmail: user.email!,
    entityType: 'vaccination',
    entityId: row.id,
    action: 'created',
    diffJson: { personId: parsed.data.personId, vaccineType: parsed.data.vaccineType },
  });

  return NextResponse.json({ vaccination: row }, { status: 201 });
});
